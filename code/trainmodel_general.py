...
from utils.data_utils import DatasetFromFolder_mri_general
...
#from dp_model.model_files import resnet3d_oasis as resnet3d
...
def trainmodel(cfg):
    ...
    if cfg.NET_DATALOADER.DATASET == 'abide':
        from dp_model.model_files import resnet3d_abide as resnet3d
        logger.info('load resnet 34 model for abide...')
    elif cfg.NET_DATALOADER.DATASET == 'oasis':
        from dp_model.model_files import resnet3d_oasis as resnet3d
        logger.info('load resnet 34 model for oasis...')
    ...
    image_file_list = list(np.load(cfg.NET_DATALOADER.TRAIN_IMAGELIST_PATH, allow_pickle=True)) #image_file_list
    training_set = DatasetFromFolder_mri_general(cfg=cfg, image_dir=cfg.NET_DATALOADER.IMAGE_DIR, target_dict_path=cfg.NET_DATALOADER.TARGET_DICT, filelist=image_file_list, direction=cfg.NET_DATALOADER.DIRECTION, mode = 'train_notsave')
        ...
        for epoch in range(1 + epoch_append, cfg.NET_TRAIN.EPOCHS + 1 + epoch_append):
            ...
            for iteration, batch in enumerate(training_data_loader, 1):
                ...
                if cfg.NET_MODEL.TYPE == 'SFCN':
                    ...
                    #print('targte size:', target.size())
                    #print('output size:', net_output.size())
                    target = target.reshape([net_output.size()[0], net_output.size()[1]])
