...
predict_save_path = '../../statistic_methods_predict/elasticnet/predict_data.npz'
csv_save_path = '../../statistic_methods_predict/elasticnet/ukbiobank'
model_save_dir = '../../statistic_methods_predict/elasticnet/'

logger = setup_logger("elasticnet sparse", model_save_dir, if_train=True)

pca_data_path = pca_cfg.PCA.DATA_SAVE_PATH

#sparse_matrix_data = sp.load_npz(pca_cfg.PCA.DATA_SAVE_PATH)
...
clf = make_pipeline(StandardScaler(with_mean=True), ElasticNet(alpha=0.1, l1_ratio = 0.00000000001, random_state=0, max_iter = 100000, warm_start=False, tol=1e-6))
