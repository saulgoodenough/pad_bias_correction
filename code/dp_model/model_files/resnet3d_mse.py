...
class ResNet(nn.Module):
    def __init__(self, block, layers, num_classes=1000,
        ...
        #net.add_module('avgpool', nn.AvgPool3d([3, 6, 5], stride=1))  #
        if dropout is True:
            net.add_module('dropout', nn.Dropout(0.5))
        self.feature_extractor = net
        self.regressor = nn.Sequential(nn.Linear(c[4] * block.expansion, 16),
                                          nn.LeakyReLU(),
                                          nn.Linear(16, num_classes) ) #nn.Linear(c[4] * block.expansion, num_classes)

        self.bin_range = [float(bin_range[0]), float(bin_range[1])]
    ...
    def forward(self, x):
        ...
        x = x.view(x.size(0), -1)
        x = self.regressor(x)
        #x = F.tanh(x) # mse: softmax
        #x = x * (self.bin_range[1] - self.bin_range[0])*1/2 + self.bin_range[0]+1/2*(self.bin_range[1] - self.bin_range[0])
        x = x * (self.bin_range[1] - self.bin_range[0]) + self.bin_range[0]
        return x
