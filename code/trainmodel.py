...
from utils.loss import age_diff, age_diff_crossentropy
...
from dp_model.model_files import sfcn as sfcn

from dp_model.model_files import resnet3d
from dp_model.model_files import resnet3d_leftright as resnet3d_leftright

from dp_model.model_files import msd3d
...
def trainmodel(cfg):
    ...
    if cfg.SOLVER.LOSS == 'NLLLoss':
        from dp_model.model_files import resnet3d_crossentropy as resnet3d
        print('Using cross entropy model.')
    ...
    if cfg.NET_TRAIN.TRAIN_TYPE == 'whole':
        ...
        for epoch in range(1 + epoch_append, cfg.NET_TRAIN.EPOCHS + 1 + epoch_append):
            ...
            for iteration, batch in enumerate(training_data_loader, 1):
                ...
                if cfg.SOLVER.LOSS == 'NLLLoss':
                    #print('target age shape:', target_age.shape)
                    #target_age_shape = target_age.shape
                    target_age = target_age.view(cfg.NET_DATALOADER.BATCH_SIZE)  - cfg.NET_DATALOADER.BIN_RANGE[0]
                    #print(target_age)
                    target_age = target_age.long()
                ...
                # print(target.size())
                if cfg.SOLVER.LOSS == 'NLLLoss':
                    #print('input', torch.argmax(net_output, axis=1), target_age)
                    loss = criterion(net_output, target_age)
                    age_x, age_difference = age_diff_crossentropy(net_output, target_age, cfg.NET_DATALOADER.BIN_RANGE)
                else:
            ...
            with torch.no_grad():
                for iteration, batch in enumerate(validate_data_loader, 1):
                    ...
                    if cfg.SOLVER.LOSS == 'NLLLoss':
                        target_age = target_age.view(cfg.NET_DATALOADER.TEST_BATCH_SIZE) - cfg.NET_DATALOADER.BIN_RANGE[0]
                        #print(target_age)
                        target_age = target_age.long()
                        #print('target shape', target_age.shape)

                    net_output = Net(input)
                    #print(net_output.shape)

                    #test_loss = criterion(net_output, target)
                    if cfg.SOLVER.LOSS == 'NLLLoss':
                    ...
                    if cfg.SOLVER.LOSS == 'NLLLoss':
                        target_age = target_age + cfg.NET_DATALOADER.BIN_RANGE[0]
        ...
        logger.info('===> start predicting the training set')
        ...
        with torch.no_grad():
            for iteration, batch in enumerate(training_data_loader, 1):
                ...
                training_epoch_loss += test_loss.item()
                age_diff_loss += age_difference.item()
                if cfg.SOLVER.LOSS == 'NLLLoss':
                    target_age = target_age + cfg.NET_DATALOADER.BIN_RANGE[0]
                writer.writerow([userid, age_x.item(), target_age.item(), age_difference.item()])
        ...
        logger.info("WHOLE ===> Epoch {} Complete: Avg. Trainig Loss: {:.8f}".format(epoch, avg_training_loss))
        logger.info("WHOLE ===> Epoch {} Complete: Avg. Trainig age difference: {:.8f}".format(epoch, ave_age_loss))
